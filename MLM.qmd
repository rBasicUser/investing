...
dir.create("plots/MLM", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_MLM.png", "MonteCarlo_MLM.png", "Regression_MLM.png", "SMA_MLM.png")

# Copiar imágenes desde output/plots/MLM/ a plots/MLM/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "MLM", img)
  destino <- file.path("plots", "MLM", img)
...
serie_original <- read_csv("data/processed/MLM_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_MLM.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_MLM.rds")
model_regression <- readRDS("models/Regression/model_regression_MLM.rds")
model_SMA <- readRDS("models/SMA/model_SMA_MLM.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - MLM",
    subtitle = paste(
...
knitr::include_graphics("plots/MLM/ARIMA_MLM.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/MLM/MonteCarlo_MLM.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/MLM/Regression_MLM.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/MLM/SMA_MLM.png")
```
...
-   `output/tables/MLM/model_MonteCarlo_metrics.csv`
-   `output/tables/MLM/model_ARIMA_metrics.csv`
-   `output/tables/MLM/model_Regression_metrics.csv`
-   `output/tables/MLM/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "MLM", paste0("model_", modelo, "_metrics.csv"))
