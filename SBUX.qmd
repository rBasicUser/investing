...
dir.create("plots/SBUX", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_SBUX.png", "MonteCarlo_SBUX.png", "Regression_SBUX.png", "SMA_SBUX.png")

# Copiar imágenes desde output/plots/SBUX/ a plots/SBUX/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "SBUX", img)
  destino <- file.path("plots", "SBUX", img)
...
serie_original <- read_csv("data/processed/SBUX_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_SBUX.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_SBUX.rds")
model_regression <- readRDS("models/Regression/model_regression_SBUX.rds")
model_SMA <- readRDS("models/SMA/model_SMA_SBUX.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - SBUX",
    subtitle = paste(
...
knitr::include_graphics("plots/SBUX/ARIMA_SBUX.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/SBUX/MonteCarlo_SBUX.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/SBUX/Regression_SBUX.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/SBUX/SMA_SBUX.png")
```
...
-   `output/tables/SBUX/model_MonteCarlo_metrics.csv`
-   `output/tables/SBUX/model_ARIMA_metrics.csv`
-   `output/tables/SBUX/model_Regression_metrics.csv`
-   `output/tables/SBUX/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "SBUX", paste0("model_", modelo, "_metrics.csv"))
