...
dir.create("plots/C", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_C.png", "MonteCarlo_C.png", "Regression_C.png", "SMA_C.png")

# Copiar imágenes desde output/plots/C/ a plots/C/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "C", img)
  destino <- file.path("plots", "C", img)
...
serie_original <- read_csv("data/processed/C_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_C.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_C.rds")
model_regression <- readRDS("models/Regression/model_regression_C.rds")
model_SMA <- readRDS("models/SMA/model_SMA_C.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - C",
    subtitle = paste(
...
knitr::include_graphics("plots/C/ARIMA_C.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/C/MonteCarlo_C.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/C/Regression_C.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/C/SMA_C.png")
```
...
-   `output/tables/C/model_MonteCarlo_metrics.csv`
-   `output/tables/C/model_ARIMA_metrics.csv`
-   `output/tables/C/model_Regression_metrics.csv`
-   `output/tables/C/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "C", paste0("model_", modelo, "_metrics.csv"))
