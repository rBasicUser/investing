...
dir.create("plots/MET", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_MET.png", "MonteCarlo_MET.png", "Regression_MET.png", "SMA_MET.png")

# Copiar imágenes desde output/plots/MET/ a plots/MET/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "MET", img)
  destino <- file.path("plots", "MET", img)
...
serie_original <- read_csv("data/processed/MET_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_MET.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_MET.rds")
model_regression <- readRDS("models/Regression/model_regression_MET.rds")
model_SMA <- readRDS("models/SMA/model_SMA_MET.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - MET",
    subtitle = paste(
...
knitr::include_graphics("plots/MET/ARIMA_MET.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/MET/MonteCarlo_MET.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/MET/Regression_MET.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/MET/SMA_MET.png")
```
...
-   `output/tables/MET/model_MonteCarlo_metrics.csv`
-   `output/tables/MET/model_ARIMA_metrics.csv`
-   `output/tables/MET/model_Regression_metrics.csv`
-   `output/tables/MET/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "MET", paste0("model_", modelo, "_metrics.csv"))
