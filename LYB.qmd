...
dir.create("plots/LYB", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_LYB.png", "MonteCarlo_LYB.png", "Regression_LYB.png", "SMA_LYB.png")

# Copiar imágenes desde output/plots/LYB/ a plots/LYB/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "LYB", img)
  destino <- file.path("plots", "LYB", img)
...
serie_original <- read_csv("data/processed/LYB_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_LYB.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_LYB.rds")
model_regression <- readRDS("models/Regression/model_regression_LYB.rds")
model_SMA <- readRDS("models/SMA/model_SMA_LYB.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - LYB",
    subtitle = paste(
...
knitr::include_graphics("plots/LYB/ARIMA_LYB.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/LYB/MonteCarlo_LYB.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/LYB/Regression_LYB.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/LYB/SMA_LYB.png")
```
...
-   `output/tables/LYB/model_MonteCarlo_metrics.csv`
-   `output/tables/LYB/model_ARIMA_metrics.csv`
-   `output/tables/LYB/model_Regression_metrics.csv`
-   `output/tables/LYB/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "LYB", paste0("model_", modelo, "_metrics.csv"))
