...
dir.create("plots/RTX", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_RTX.png", "MonteCarlo_RTX.png", "Regression_RTX.png", "SMA_RTX.png")

# Copiar imágenes desde output/plots/RTX/ a plots/RTX/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "RTX", img)
  destino <- file.path("plots", "RTX", img)
...
serie_original <- read_csv("data/processed/RTX_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_RTX.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_RTX.rds")
model_regression <- readRDS("models/Regression/model_regression_RTX.rds")
model_SMA <- readRDS("models/SMA/model_SMA_RTX.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - RTX",
    subtitle = paste(
...
knitr::include_graphics("plots/RTX/ARIMA_RTX.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/RTX/MonteCarlo_RTX.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/RTX/Regression_RTX.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/RTX/SMA_RTX.png")
```
...
-   `output/tables/RTX/model_MonteCarlo_metrics.csv`
-   `output/tables/RTX/model_ARIMA_metrics.csv`
-   `output/tables/RTX/model_Regression_metrics.csv`
-   `output/tables/RTX/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "RTX", paste0("model_", modelo, "_metrics.csv"))
