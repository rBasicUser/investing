...
dir.create("plots/TSLA", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_TSLA.png", "MonteCarlo_TSLA.png", "Regression_TSLA.png", "SMA_TSLA.png")

# Copiar imágenes desde output/plots/TSLA/ a plots/TSLA/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "TSLA", img)
  destino <- file.path("plots", "TSLA", img)
...
serie_original <- read_csv("data/processed/TSLA_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_TSLA.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_TSLA.rds")
model_regression <- readRDS("models/Regression/model_regression_TSLA.rds")
model_SMA <- readRDS("models/SMA/model_SMA_TSLA.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - TSLA",
    subtitle = paste(
...
knitr::include_graphics("plots/TSLA/ARIMA_TSLA.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/TSLA/MonteCarlo_TSLA.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/TSLA/Regression_TSLA.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/TSLA/SMA_TSLA.png")
```
...
-   `output/tables/TSLA/model_MonteCarlo_metrics.csv`
-   `output/tables/TSLA/model_ARIMA_metrics.csv`
-   `output/tables/TSLA/model_Regression_metrics.csv`
-   `output/tables/TSLA/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "TSLA", paste0("model_", modelo, "_metrics.csv"))
