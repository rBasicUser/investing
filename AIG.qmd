...
dir.create("plots/AIG", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_AIG.png", "MonteCarlo_AIG.png", "Regression_AIG.png", "SMA_AIG.png")

# Copiar imágenes desde output/plots/AIG/ a plots/AIG/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "AIG", img)
  destino <- file.path("plots", "AIG", img)
...
serie_original <- read_csv("data/processed/AIG_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_AIG.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_AIG.rds")
model_regression <- readRDS("models/Regression/model_regression_AIG.rds")
model_SMA <- readRDS("models/SMA/model_SMA_AIG.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - AIG",
    subtitle = paste(
...
knitr::include_graphics("plots/AIG/ARIMA_AIG.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/AIG/MonteCarlo_AIG.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/AIG/Regression_AIG.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/AIG/SMA_AIG.png")
```
...
-   `output/tables/AIG/model_MonteCarlo_metrics.csv`
-   `output/tables/AIG/model_ARIMA_metrics.csv`
-   `output/tables/AIG/model_Regression_metrics.csv`
-   `output/tables/AIG/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "AIG", paste0("model_", modelo, "_metrics.csv"))
