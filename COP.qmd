...
dir.create("plots/COP", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_COP.png", "MonteCarlo_COP.png", "Regression_COP.png", "SMA_COP.png")

# Copiar imágenes desde output/plots/COP/ a plots/COP/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "COP", img)
  destino <- file.path("plots", "COP", img)
...
serie_original <- read_csv("data/processed/COP_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_COP.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_COP.rds")
model_regression <- readRDS("models/Regression/model_regression_COP.rds")
model_SMA <- readRDS("models/SMA/model_SMA_COP.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - COP",
    subtitle = paste(
...
knitr::include_graphics("plots/COP/ARIMA_COP.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/COP/MonteCarlo_COP.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/COP/Regression_COP.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/COP/SMA_COP.png")
```
...
-   `output/tables/COP/model_MonteCarlo_metrics.csv`
-   `output/tables/COP/model_ARIMA_metrics.csv`
-   `output/tables/COP/model_Regression_metrics.csv`
-   `output/tables/COP/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "COP", paste0("model_", modelo, "_metrics.csv"))
