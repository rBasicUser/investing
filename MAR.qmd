...
dir.create("plots/MAR", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_MAR.png", "MonteCarlo_MAR.png", "Regression_MAR.png", "SMA_MAR.png")

# Copiar imágenes desde output/plots/MAR/ a plots/MAR/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "MAR", img)
  destino <- file.path("plots", "MAR", img)
...
serie_original <- read_csv("data/processed/MAR_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_MAR.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_MAR.rds")
model_regression <- readRDS("models/Regression/model_regression_MAR.rds")
model_SMA <- readRDS("models/SMA/model_SMA_MAR.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - MAR",
    subtitle = paste(
...
knitr::include_graphics("plots/MAR/ARIMA_MAR.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/MAR/MonteCarlo_MAR.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/MAR/Regression_MAR.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/MAR/SMA_MAR.png")
```
...
-   `output/tables/MAR/model_MonteCarlo_metrics.csv`
-   `output/tables/MAR/model_ARIMA_metrics.csv`
-   `output/tables/MAR/model_Regression_metrics.csv`
-   `output/tables/MAR/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "MAR", paste0("model_", modelo, "_metrics.csv"))
