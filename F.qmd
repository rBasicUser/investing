...
dir.create("plots/F", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_F.png", "MonteCarlo_F.png", "Regression_F.png", "SMA_F.png")

# Copiar imágenes desde output/plots/F/ a plots/F/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "F", img)
  destino <- file.path("plots", "F", img)
...
serie_original <- read_csv("data/processed/F_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_F.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_F.rds")
model_regression <- readRDS("models/Regression/model_regression_F.rds")
model_SMA <- readRDS("models/SMA/model_SMA_F.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - F",
    subtitle = paste(
...
knitr::include_graphics("plots/F/ARIMA_F.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/F/MonteCarlo_F.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/F/Regression_F.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/F/SMA_F.png")
```
...
-   `output/tables/F/model_MonteCarlo_metrics.csv`
-   `output/tables/F/model_ARIMA_metrics.csv`
-   `output/tables/F/model_Regression_metrics.csv`
-   `output/tables/F/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "F", paste0("model_", modelo, "_metrics.csv"))
