...
dir.create("plots/GM", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_GM.png", "MonteCarlo_GM.png", "Regression_GM.png", "SMA_GM.png")

# Copiar imágenes desde output/plots/GM/ a plots/GM/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "GM", img)
  destino <- file.path("plots", "GM", img)
...
serie_original <- read_csv("data/processed/GM_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_GM.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_GM.rds")
model_regression <- readRDS("models/Regression/model_regression_GM.rds")
model_SMA <- readRDS("models/SMA/model_SMA_GM.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - GM",
    subtitle = paste(
...
knitr::include_graphics("plots/GM/ARIMA_GM.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/GM/MonteCarlo_GM.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/GM/Regression_GM.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/GM/SMA_GM.png")
```
...
-   `output/tables/GM/model_MonteCarlo_metrics.csv`
-   `output/tables/GM/model_ARIMA_metrics.csv`
-   `output/tables/GM/model_Regression_metrics.csv`
-   `output/tables/GM/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "GM", paste0("model_", modelo, "_metrics.csv"))
