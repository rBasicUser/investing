...
dir.create("plots/FCX", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_FCX.png", "MonteCarlo_FCX.png", "Regression_FCX.png", "SMA_FCX.png")

# Copiar imágenes desde output/plots/FCX/ a plots/FCX/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "FCX", img)
  destino <- file.path("plots", "FCX", img)
...
serie_original <- read_csv("data/processed/FCX_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_FCX.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_FCX.rds")
model_regression <- readRDS("models/Regression/model_regression_FCX.rds")
model_SMA <- readRDS("models/SMA/model_SMA_FCX.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - FCX",
    subtitle = paste(
...
knitr::include_graphics("plots/FCX/ARIMA_FCX.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/FCX/MonteCarlo_FCX.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/FCX/Regression_FCX.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/FCX/SMA_FCX.png")
```
...
-   `output/tables/FCX/model_MonteCarlo_metrics.csv`
-   `output/tables/FCX/model_ARIMA_metrics.csv`
-   `output/tables/FCX/model_Regression_metrics.csv`
-   `output/tables/FCX/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "FCX", paste0("model_", modelo, "_metrics.csv"))
