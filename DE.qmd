...
dir.create("plots/DE", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_DE.png", "MonteCarlo_DE.png", "Regression_DE.png", "SMA_DE.png")

# Copiar imágenes desde output/plots/DE/ a plots/DE/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "DE", img)
  destino <- file.path("plots", "DE", img)
...
serie_original <- read_csv("data/processed/DE_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_DE.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_DE.rds")
model_regression <- readRDS("models/Regression/model_regression_DE.rds")
model_SMA <- readRDS("models/SMA/model_SMA_DE.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - DE",
    subtitle = paste(
...
knitr::include_graphics("plots/DE/ARIMA_DE.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/DE/MonteCarlo_DE.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/DE/Regression_DE.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/DE/SMA_DE.png")
```
...
-   `output/tables/DE/model_MonteCarlo_metrics.csv`
-   `output/tables/DE/model_ARIMA_metrics.csv`
-   `output/tables/DE/model_Regression_metrics.csv`
-   `output/tables/DE/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "DE", paste0("model_", modelo, "_metrics.csv"))
