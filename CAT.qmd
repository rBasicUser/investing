...
dir.create("plots/CAT", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_CAT.png", "MonteCarlo_CAT.png", "Regression_CAT.png", "SMA_CAT.png")

# Copiar imágenes desde output/plots/CAT/ a plots/CAT/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "CAT", img)
  destino <- file.path("plots", "CAT", img)
...
serie_original <- read_csv("data/processed/CAT_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_CAT.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_CAT.rds")
model_regression <- readRDS("models/Regression/model_regression_CAT.rds")
model_SMA <- readRDS("models/SMA/model_SMA_CAT.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - CAT",
    subtitle = paste(
...
knitr::include_graphics("plots/CAT/ARIMA_CAT.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/CAT/MonteCarlo_CAT.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/CAT/Regression_CAT.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/CAT/SMA_CAT.png")
```
...
-   `output/tables/CAT/model_MonteCarlo_metrics.csv`
-   `output/tables/CAT/model_ARIMA_metrics.csv`
-   `output/tables/CAT/model_Regression_metrics.csv`
-   `output/tables/CAT/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "CAT", paste0("model_", modelo, "_metrics.csv"))
