...
dir.create("plots/VMC", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_VMC.png", "MonteCarlo_VMC.png", "Regression_VMC.png", "SMA_VMC.png")

# Copiar imágenes desde output/plots/VMC/ a plots/VMC/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "VMC", img)
  destino <- file.path("plots", "VMC", img)
...
serie_original <- read_csv("data/processed/VMC_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_VMC.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_VMC.rds")
model_regression <- readRDS("models/Regression/model_regression_VMC.rds")
model_SMA <- readRDS("models/SMA/model_SMA_VMC.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - VMC",
    subtitle = paste(
...
knitr::include_graphics("plots/VMC/ARIMA_VMC.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/VMC/MonteCarlo_VMC.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/VMC/Regression_VMC.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/VMC/SMA_VMC.png")
```
...
-   `output/tables/VMC/model_MonteCarlo_metrics.csv`
-   `output/tables/VMC/model_ARIMA_metrics.csv`
-   `output/tables/VMC/model_Regression_metrics.csv`
-   `output/tables/VMC/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "VMC", paste0("model_", modelo, "_metrics.csv"))
