...
dir.create("plots/HAL", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_HAL.png", "MonteCarlo_HAL.png", "Regression_HAL.png", "SMA_HAL.png")

# Copiar imágenes desde output/plots/HAL/ a plots/HAL/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "HAL", img)
  destino <- file.path("plots", "HAL", img)
...
serie_original <- read_csv("data/processed/HAL_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_HAL.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_HAL.rds")
model_regression <- readRDS("models/Regression/model_regression_HAL.rds")
model_SMA <- readRDS("models/SMA/model_SMA_HAL.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - HAL",
    subtitle = paste(
...
knitr::include_graphics("plots/HAL/ARIMA_HAL.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/HAL/MonteCarlo_HAL.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/HAL/Regression_HAL.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/HAL/SMA_HAL.png")
```
...
-   `output/tables/HAL/model_MonteCarlo_metrics.csv`
-   `output/tables/HAL/model_ARIMA_metrics.csv`
-   `output/tables/HAL/model_Regression_metrics.csv`
-   `output/tables/HAL/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "HAL", paste0("model_", modelo, "_metrics.csv"))
