...
dir.create("plots/NUE", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_NUE.png", "MonteCarlo_NUE.png", "Regression_NUE.png", "SMA_NUE.png")

# Copiar imágenes desde output/plots/NUE/ a plots/NUE/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "NUE", img)
  destino <- file.path("plots", "NUE", img)
...
serie_original <- read_csv("data/processed/NUE_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_NUE.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_NUE.rds")
model_regression <- readRDS("models/Regression/model_regression_NUE.rds")
model_SMA <- readRDS("models/SMA/model_SMA_NUE.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - NUE",
    subtitle = paste(
...
knitr::include_graphics("plots/NUE/ARIMA_NUE.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/NUE/MonteCarlo_NUE.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/NUE/Regression_NUE.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/NUE/SMA_NUE.png")
```
...
-   `output/tables/NUE/model_MonteCarlo_metrics.csv`
-   `output/tables/NUE/model_ARIMA_metrics.csv`
-   `output/tables/NUE/model_Regression_metrics.csv`
-   `output/tables/NUE/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "NUE", paste0("model_", modelo, "_metrics.csv"))
