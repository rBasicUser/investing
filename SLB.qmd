...
dir.create("plots/SLB", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_SLB.png", "MonteCarlo_SLB.png", "Regression_SLB.png", "SMA_SLB.png")

# Copiar imágenes desde output/plots/SLB/ a plots/SLB/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "SLB", img)
  destino <- file.path("plots", "SLB", img)
...
serie_original <- read_csv("data/processed/SLB_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_SLB.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_SLB.rds")
model_regression <- readRDS("models/Regression/model_regression_SLB.rds")
model_SMA <- readRDS("models/SMA/model_SMA_SLB.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - SLB",
    subtitle = paste(
...
knitr::include_graphics("plots/SLB/ARIMA_SLB.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/SLB/MonteCarlo_SLB.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/SLB/Regression_SLB.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/SLB/SMA_SLB.png")
```
...
-   `output/tables/SLB/model_MonteCarlo_metrics.csv`
-   `output/tables/SLB/model_ARIMA_metrics.csv`
-   `output/tables/SLB/model_Regression_metrics.csv`
-   `output/tables/SLB/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "SLB", paste0("model_", modelo, "_metrics.csv"))
