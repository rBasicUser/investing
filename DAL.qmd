...
dir.create("plots/DAL", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_DAL.png", "MonteCarlo_DAL.png", "Regression_DAL.png", "SMA_DAL.png")

# Copiar imágenes desde output/plots/DAL/ a plots/DAL/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "DAL", img)
  destino <- file.path("plots", "DAL", img)
...
serie_original <- read_csv("data/processed/DAL_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_DAL.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_DAL.rds")
model_regression <- readRDS("models/Regression/model_regression_DAL.rds")
model_SMA <- readRDS("models/SMA/model_SMA_DAL.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - DAL",
    subtitle = paste(
...
knitr::include_graphics("plots/DAL/ARIMA_DAL.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/DAL/MonteCarlo_DAL.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/DAL/Regression_DAL.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/DAL/SMA_DAL.png")
```
...
-   `output/tables/DAL/model_MonteCarlo_metrics.csv`
-   `output/tables/DAL/model_ARIMA_metrics.csv`
-   `output/tables/DAL/model_Regression_metrics.csv`
-   `output/tables/DAL/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "DAL", paste0("model_", modelo, "_metrics.csv"))
