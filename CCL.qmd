...
dir.create("plots/CCL", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_CCL.png", "MonteCarlo_CCL.png", "Regression_CCL.png", "SMA_CCL.png")

# Copiar imágenes desde output/plots/CCL/ a plots/CCL/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "CCL", img)
  destino <- file.path("plots", "CCL", img)
...
serie_original <- read_csv("data/processed/CCL_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_CCL.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_CCL.rds")
model_regression <- readRDS("models/Regression/model_regression_CCL.rds")
model_SMA <- readRDS("models/SMA/model_SMA_CCL.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - CCL",
    subtitle = paste(
...
knitr::include_graphics("plots/CCL/ARIMA_CCL.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/CCL/MonteCarlo_CCL.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/CCL/Regression_CCL.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/CCL/SMA_CCL.png")
```
...
-   `output/tables/CCL/model_MonteCarlo_metrics.csv`
-   `output/tables/CCL/model_ARIMA_metrics.csv`
-   `output/tables/CCL/model_Regression_metrics.csv`
-   `output/tables/CCL/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "CCL", paste0("model_", modelo, "_metrics.csv"))
