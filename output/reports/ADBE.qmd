...
dir.create("plots/ADBE", recursive = TRUE, showWarnings = FALSE)

imagenes <- c("ARIMA_ADBE.png", "MonteCarlo_ADBE.png", "Regression_ADBE.png", "SMA_ADBE.png")

for (img in imagenes) {
  file.copy(paste0("../plots/ADBE/", img), paste0("plots/ADBE/", img), overwrite = TRUE)
  }
```
...
serie_original <- read_csv("../../data/processed/ADBE_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos
model_montecarlo <- readRDS("../../models/MonteCarlo/model_montecarlo_ADBE.rds")
model_ARIMA <- readRDS("../../models/ARIMA/model_ARIMA_ADBE.rds")
model_regression <- readRDS("../../models/Regression/model_regression_ADBE.rds")
model_SMA <- readRDS("../../models/SMA/model_SMA_ADBE.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - ADBE",
    subtitle = paste(
...
knitr::include_graphics("plots/ADBE/ARIMA_ADBE.png")
```
...
knitr::include_graphics("plots/ADBE/MonteCarlo_ADBE.png")
```
...
knitr::include_graphics("plots/ADBE/Regression_ADBE.png")
```
...
knitr::include_graphics("plots/ADBE/SMA_ADBE.png")
```
...
-   `output/tables/ADBE/model_MonteCarlo_metrics.csv`
-   `output/tables/ADBE/model_ARIMA_metrics.csv`
-   `output/tables/ADBE/model_Regression_metrics.csv`
-   `output/tables/ADBE/model_SMA_metrics.csv`
...
#   function(modelo) {
#     archivo <- paste0("output/tables/ADBE/model_", modelo, "_metrics.csv")
