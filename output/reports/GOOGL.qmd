...
dir.create("plots/GOOGL", recursive = TRUE, showWarnings = FALSE)

imagenes <- c("ARIMA_GOOGL.png", "MonteCarlo_GOOGL.png", "Regression_GOOGL.png", "SMA_GOOGL.png")

for (img in imagenes) {
  file.copy(paste0("../plots/GOOGL/", img), paste0("plots/GOOGL/", img), overwrite = TRUE)
  }
```
...
serie_original <- read_csv("../../data/processed/GOOGL_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos
model_montecarlo <- readRDS("../../models/MonteCarlo/model_montecarlo_GOOGL.rds")
model_ARIMA <- readRDS("../../models/ARIMA/model_ARIMA_GOOGL.rds")
model_regression <- readRDS("../../models/Regression/model_regression_GOOGL.rds")
model_SMA <- readRDS("../../models/SMA/model_SMA_GOOGL.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - GOOGL",
    subtitle = paste(
...
knitr::include_graphics("plots/GOOGL/ARIMA_GOOGL.png")
```
...
knitr::include_graphics("plots/GOOGL/MonteCarlo_GOOGL.png")
```
...
knitr::include_graphics("plots/GOOGL/Regression_GOOGL.png")
```
...
knitr::include_graphics("plots/GOOGL/SMA_GOOGL.png")
```
...
-   `output/tables/GOOGL/model_MonteCarlo_metrics.csv`
-   `output/tables/GOOGL/model_ARIMA_metrics.csv`
-   `output/tables/GOOGL/model_Regression_metrics.csv`
-   `output/tables/GOOGL/model_SMA_metrics.csv`
...
#   function(modelo) {
#     archivo <- paste0("output/tables/GOOGL/model_", modelo, "_metrics.csv")
