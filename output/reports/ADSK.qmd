...
dir.create("plots/ADSK", recursive = TRUE, showWarnings = FALSE)

imagenes <- c("ARIMA_ADSK.png", "MonteCarlo_ADSK.png", "Regression_ADSK.png", "SMA_ADSK.png")

for (img in imagenes) {
  file.copy(paste0("../plots/ADSK/", img), paste0("plots/ADSK/", img), overwrite = TRUE)
  }
```
...
serie_original <- read_csv("../../data/processed/ADSK_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos
model_montecarlo <- readRDS("../../models/MonteCarlo/model_montecarlo_ADSK.rds")
model_ARIMA <- readRDS("../../models/ARIMA/model_ARIMA_ADSK.rds")
model_regression <- readRDS("../../models/Regression/model_regression_ADSK.rds")
model_SMA <- readRDS("../../models/SMA/model_SMA_ADSK.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - ADSK",
    subtitle = paste(
...
knitr::include_graphics("plots/ADSK/ARIMA_ADSK.png")
```
...
knitr::include_graphics("plots/ADSK/MonteCarlo_ADSK.png")
```
...
knitr::include_graphics("plots/ADSK/Regression_ADSK.png")
```
...
knitr::include_graphics("plots/ADSK/SMA_ADSK.png")
```
...
-   `output/tables/ADSK/model_MonteCarlo_metrics.csv`
-   `output/tables/ADSK/model_ARIMA_metrics.csv`
-   `output/tables/ADSK/model_Regression_metrics.csv`
-   `output/tables/ADSK/model_SMA_metrics.csv`
...
#   function(modelo) {
#     archivo <- paste0("output/tables/ADSK/model_", modelo, "_metrics.csv")
