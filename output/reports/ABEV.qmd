...
dir.create("plots/ABEV", recursive = TRUE, showWarnings = FALSE)

imagenes <- c("ARIMA_ABEV.png", "MonteCarlo_ABEV.png", "Regression_ABEV.png", "SMA_ABEV.png")

for (img in imagenes) {
  file.copy(paste0("../plots/ABEV/", img), paste0("plots/ABEV/", img), overwrite = TRUE)
  }
```
...
serie_original <- read_csv("../../data/processed/ABEV_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos
model_montecarlo <- readRDS("../../models/MonteCarlo/model_montecarlo_ABEV.rds")
model_ARIMA <- readRDS("../../models/ARIMA/model_ARIMA_ABEV.rds")
model_regression <- readRDS("../../models/Regression/model_regression_ABEV.rds")
model_SMA <- readRDS("../../models/SMA/model_SMA_ABEV.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - ABEV",
    subtitle = paste(
...
knitr::include_graphics("plots/ABEV/ARIMA_ABEV.png")
```
...
knitr::include_graphics("plots/ABEV/MonteCarlo_ABEV.png")
```
...
knitr::include_graphics("plots/ABEV/Regression_ABEV.png")
```
...
knitr::include_graphics("plots/ABEV/SMA_ABEV.png")
```
...
-   `output/tables/ABEV/model_MonteCarlo_metrics.csv`
-   `output/tables/ABEV/model_ARIMA_metrics.csv`
-   `output/tables/ABEV/model_Regression_metrics.csv`
-   `output/tables/ABEV/model_SMA_metrics.csv`
...
#   function(modelo) {
#     archivo <- paste0("output/tables/ABEV/model_", modelo, "_metrics.csv")
