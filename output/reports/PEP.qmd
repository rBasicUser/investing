...
dir.create("plots/PEP", recursive = TRUE, showWarnings = FALSE)

imagenes <- c("ARIMA_PEP.png", "MonteCarlo_PEP.png", "Regression_PEP.png", "SMA_PEP.png")

for (img in imagenes) {
  file.copy(paste0("../plots/PEP/", img), paste0("plots/PEP/", img), overwrite = TRUE)
  }
```
...
serie_original <- read_csv("../../data/processed/PEP_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos
model_montecarlo <- readRDS("../../models/MonteCarlo/model_montecarlo_PEP.rds")
model_ARIMA <- readRDS("../../models/ARIMA/model_ARIMA_PEP.rds")
model_regression <- readRDS("../../models/Regression/model_regression_PEP.rds")
model_SMA <- readRDS("../../models/SMA/model_SMA_PEP.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - PEP",
    subtitle = paste(
...
knitr::include_graphics("plots/PEP/ARIMA_PEP.png")
```
...
knitr::include_graphics("plots/PEP/MonteCarlo_PEP.png")
```
...
knitr::include_graphics("plots/PEP/Regression_PEP.png")
```
...
knitr::include_graphics("plots/PEP/SMA_PEP.png")
```
...
-   `output/tables/PEP/model_MonteCarlo_metrics.csv`
-   `output/tables/PEP/model_ARIMA_metrics.csv`
-   `output/tables/PEP/model_Regression_metrics.csv`
-   `output/tables/PEP/model_SMA_metrics.csv`
...
#   function(modelo) {
#     archivo <- paste0("output/tables/PEP/model_", modelo, "_metrics.csv")
