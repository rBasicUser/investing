...
dir.create("plots/WMT", recursive = TRUE, showWarnings = FALSE)

imagenes <- c("ARIMA_WMT.png", "MonteCarlo_WMT.png", "Regression_WMT.png", "SMA_WMT.png")

for (img in imagenes) {
  file.copy(paste0("../plots/WMT/", img), paste0("plots/WMT/", img), overwrite = TRUE)
  }
```
...
serie_original <- read_csv("../../data/processed/WMT_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos
model_montecarlo <- readRDS("../../models/MonteCarlo/model_montecarlo_WMT.rds")
model_ARIMA <- readRDS("../../models/ARIMA/model_ARIMA_WMT.rds")
model_regression <- readRDS("../../models/Regression/model_regression_WMT.rds")
model_SMA <- readRDS("../../models/SMA/model_SMA_WMT.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - WMT",
    subtitle = paste(
...
knitr::include_graphics("plots/WMT/ARIMA_WMT.png")
```
...
knitr::include_graphics("plots/WMT/MonteCarlo_WMT.png")
```
...
knitr::include_graphics("plots/WMT/Regression_WMT.png")
```
...
knitr::include_graphics("plots/WMT/SMA_WMT.png")
```
...
-   `output/tables/WMT/model_MonteCarlo_metrics.csv`
-   `output/tables/WMT/model_ARIMA_metrics.csv`
-   `output/tables/WMT/model_Regression_metrics.csv`
-   `output/tables/WMT/model_SMA_metrics.csv`
...
#   function(modelo) {
#     archivo <- paste0("output/tables/WMT/model_", modelo, "_metrics.csv")
