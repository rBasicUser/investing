...
dir.create("plots/DOW", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_DOW.png", "MonteCarlo_DOW.png", "Regression_DOW.png", "SMA_DOW.png")

# Copiar imágenes desde output/plots/DOW/ a plots/DOW/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "DOW", img)
  destino <- file.path("plots", "DOW", img)
...
serie_original <- read_csv("data/processed/DOW_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_DOW.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_DOW.rds")
model_regression <- readRDS("models/Regression/model_regression_DOW.rds")
model_SMA <- readRDS("models/SMA/model_SMA_DOW.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - DOW",
    subtitle = paste(
...
knitr::include_graphics("plots/DOW/ARIMA_DOW.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/DOW/MonteCarlo_DOW.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/DOW/Regression_DOW.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/DOW/SMA_DOW.png")
```
...
-   `output/tables/DOW/model_MonteCarlo_metrics.csv`
-   `output/tables/DOW/model_ARIMA_metrics.csv`
-   `output/tables/DOW/model_Regression_metrics.csv`
-   `output/tables/DOW/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "DOW", paste0("model_", modelo, "_metrics.csv"))
