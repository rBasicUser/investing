...
dir.create("plots/MT", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_MT.png", "MonteCarlo_MT.png", "Regression_MT.png", "SMA_MT.png")

# Copiar imágenes desde output/plots/MT/ a plots/MT/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "MT", img)
  destino <- file.path("plots", "MT", img)
...
serie_original <- read_csv("data/processed/MT_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_MT.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_MT.rds")
model_regression <- readRDS("models/Regression/model_regression_MT.rds")
model_SMA <- readRDS("models/SMA/model_SMA_MT.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - MT",
    subtitle = paste(
...
knitr::include_graphics("plots/MT/ARIMA_MT.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/MT/MonteCarlo_MT.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/MT/Regression_MT.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/MT/SMA_MT.png")
```
...
-   `output/tables/MT/model_MonteCarlo_metrics.csv`
-   `output/tables/MT/model_ARIMA_metrics.csv`
-   `output/tables/MT/model_Regression_metrics.csv`
-   `output/tables/MT/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "MT", paste0("model_", modelo, "_metrics.csv"))
