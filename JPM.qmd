...
dir.create("plots/JPM", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_JPM.png", "MonteCarlo_JPM.png", "Regression_JPM.png", "SMA_JPM.png")

# Copiar imágenes desde output/plots/JPM/ a plots/JPM/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "JPM", img)
  destino <- file.path("plots", "JPM", img)
...
serie_original <- read_csv("data/processed/JPM_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_JPM.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_JPM.rds")
model_regression <- readRDS("models/Regression/model_regression_JPM.rds")
model_SMA <- readRDS("models/SMA/model_SMA_JPM.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - JPM",
    subtitle = paste(
...
knitr::include_graphics("plots/JPM/ARIMA_JPM.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/JPM/MonteCarlo_JPM.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/JPM/Regression_JPM.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/JPM/SMA_JPM.png")
```
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "JPM", paste0("model_", modelo, "_metrics.csv"))
