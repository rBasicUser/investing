...
dir.create("plots/CVX", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_CVX.png", "MonteCarlo_CVX.png", "Regression_CVX.png", "SMA_CVX.png")

# Copiar imágenes desde output/plots/CVX/ a plots/CVX/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "CVX", img)
  destino <- file.path("plots", "CVX", img)
...
serie_original <- read_csv("data/processed/CVX_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_CVX.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_CVX.rds")
model_regression <- readRDS("models/Regression/model_regression_CVX.rds")
model_SMA <- readRDS("models/SMA/model_SMA_CVX.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - CVX",
    subtitle = paste(
...
knitr::include_graphics("plots/CVX/ARIMA_CVX.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/CVX/MonteCarlo_CVX.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/CVX/Regression_CVX.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/CVX/SMA_CVX.png")
```
...
-   `output/tables/CVX/model_MonteCarlo_metrics.csv`
-   `output/tables/CVX/model_ARIMA_metrics.csv`
-   `output/tables/CVX/model_Regression_metrics.csv`
-   `output/tables/CVX/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "CVX", paste0("model_", modelo, "_metrics.csv"))
