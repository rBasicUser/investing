...
dir.create("plots/BA", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_BA.png", "MonteCarlo_BA.png", "Regression_BA.png", "SMA_BA.png")

# Copiar imágenes desde output/plots/BA/ a plots/BA/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "BA", img)
  destino <- file.path("plots", "BA", img)
...
serie_original <- read_csv("data/processed/BA_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_BA.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_BA.rds")
model_regression <- readRDS("models/Regression/model_regression_BA.rds")
model_SMA <- readRDS("models/SMA/model_SMA_BA.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - BA",
    subtitle = paste(
...
knitr::include_graphics("plots/BA/ARIMA_BA.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/BA/MonteCarlo_BA.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/BA/Regression_BA.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/BA/SMA_BA.png")
```
...
-   `output/tables/BA/model_MonteCarlo_metrics.csv`
-   `output/tables/BA/model_ARIMA_metrics.csv`
-   `output/tables/BA/model_Regression_metrics.csv`
-   `output/tables/BA/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "BA", paste0("model_", modelo, "_metrics.csv"))
