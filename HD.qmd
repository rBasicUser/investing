...
dir.create("plots/HD", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_HD.png", "MonteCarlo_HD.png", "Regression_HD.png", "SMA_HD.png")

# Copiar imágenes desde output/plots/HD/ a plots/HD/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "HD", img)
  destino <- file.path("plots", "HD", img)
...
serie_original <- read_csv("data/processed/HD_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_HD.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_HD.rds")
model_regression <- readRDS("models/Regression/model_regression_HD.rds")
model_SMA <- readRDS("models/SMA/model_SMA_HD.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - HD",
    subtitle = paste(
...
knitr::include_graphics("plots/HD/ARIMA_HD.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/HD/MonteCarlo_HD.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/HD/Regression_HD.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/HD/SMA_HD.png")
```
...
-   `output/tables/HD/model_MonteCarlo_metrics.csv`
-   `output/tables/HD/model_ARIMA_metrics.csv`
-   `output/tables/HD/model_Regression_metrics.csv`
-   `output/tables/HD/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "HD", paste0("model_", modelo, "_metrics.csv"))
