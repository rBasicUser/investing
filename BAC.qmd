...
dir.create("plots/BAC", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_BAC.png", "MonteCarlo_BAC.png", "Regression_BAC.png", "SMA_BAC.png")

# Copiar imágenes desde output/plots/BAC/ a plots/BAC/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "BAC", img)
  destino <- file.path("plots", "BAC", img)
...
serie_original <- read_csv("data/processed/BAC_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_BAC.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_BAC.rds")
model_regression <- readRDS("models/Regression/model_regression_BAC.rds")
model_SMA <- readRDS("models/SMA/model_SMA_BAC.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - BAC",
    subtitle = paste(
...
knitr::include_graphics("plots/BAC/ARIMA_BAC.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/BAC/MonteCarlo_BAC.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/BAC/Regression_BAC.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/BAC/SMA_BAC.png")
```
...
-   `output/tables/BAC/model_MonteCarlo_metrics.csv`
-   `output/tables/BAC/model_ARIMA_metrics.csv`
-   `output/tables/BAC/model_Regression_metrics.csv`
-   `output/tables/BAC/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "BAC", paste0("model_", modelo, "_metrics.csv"))
