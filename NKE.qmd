...
dir.create("plots/NKE", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_NKE.png", "MonteCarlo_NKE.png", "Regression_NKE.png", "SMA_NKE.png")

# Copiar imágenes desde output/plots/NKE/ a plots/NKE/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "NKE", img)
  destino <- file.path("plots", "NKE", img)
...
serie_original <- read_csv("data/processed/NKE_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_NKE.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_NKE.rds")
model_regression <- readRDS("models/Regression/model_regression_NKE.rds")
model_SMA <- readRDS("models/SMA/model_SMA_NKE.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - NKE",
    subtitle = paste(
...
knitr::include_graphics("plots/NKE/ARIMA_NKE.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/NKE/MonteCarlo_NKE.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/NKE/Regression_NKE.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/NKE/SMA_NKE.png")
```
...
-   `output/tables/NKE/model_MonteCarlo_metrics.csv`
-   `output/tables/NKE/model_ARIMA_metrics.csv`
-   `output/tables/NKE/model_Regression_metrics.csv`
-   `output/tables/NKE/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "NKE", paste0("model_", modelo, "_metrics.csv"))
