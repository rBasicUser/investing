...
dir.create("plots/XOM", recursive = TRUE, showWarnings = FALSE)

# Lista de imágenes a copiar
imagenes <- c("ARIMA_XOM.png", "MonteCarlo_XOM.png", "Regression_XOM.png", "SMA_XOM.png")

# Copiar imágenes desde output/plots/XOM/ a plots/XOM/ (directorio local)
for (img in imagenes) {
  origen <- file.path("output", "plots", "XOM", img)
  destino <- file.path("plots", "XOM", img)
...
serie_original <- read_csv("data/processed/XOM_full.csv") |>
  rename(real = close) |>
  select(date, real, daily_return, weekly_return, monthly_return, yoy_return)

# Cargar modelos predictivos - RUTAS ACTUALIZADAS
model_montecarlo <- readRDS("models/MonteCarlo/model_montecarlo_XOM.rds")
model_ARIMA <- readRDS("models/ARIMA/model_ARIMA_XOM.rds")
model_regression <- readRDS("models/Regression/model_regression_XOM.rds")
model_SMA <- readRDS("models/SMA/model_SMA_XOM.rds") |>
...
ggplot(datos_grafico, aes(x = date)) +
  ...
  labs(
    title = "Comparación de Predicciones de Modelos - XOM",
    subtitle = paste(
...
knitr::include_graphics("plots/XOM/ARIMA_XOM.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/XOM/MonteCarlo_XOM.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/XOM/Regression_XOM.png")
```
...
# Usar ruta local copiada
knitr::include_graphics("plots/XOM/SMA_XOM.png")
```
...
-   `output/tables/XOM/model_MonteCarlo_metrics.csv`
-   `output/tables/XOM/model_ARIMA_metrics.csv`
-   `output/tables/XOM/model_Regression_metrics.csv`
-   `output/tables/XOM/model_SMA_metrics.csv`
...
metricas_todos <- map_dfr(
  ...
  function(modelo) {
    archivo <- file.path("output", "tables", "XOM", paste0("model_", modelo, "_metrics.csv"))
